...
export function DateRow({ day, weekday, weekdayFormat = 'zh', isDark = false, isToday, primaryEvent, eventCount, onClick, isEvenRow }) {
  ...
  return (
    <tr
      ...
      `}
    >
      <td className={`px-1.5 py-0.5 w-8 tabular-nums ${isToday ? (isDark ? 'text-amber-300 font-medium' : 'text-amber-800 font-medium') : (isDark ? 'text-slate-300' : 'text-slate-700')}`}>
        {day}
      </td>
      <td className={`px-1 py-0.5 w-6 text-[10px] font-medium ${weekdayColor}`}>
        {weekdayLabel}
      </td>
      <td className={`px-1.5 py-0.5 ${eventColorClass}`}>
